...
func TestTransactionList(t *testing.T) {
	...
	t1 := proto.TransferV2{Transfer: proto.Transfer{ID: &d1}}
	lst.Add(&t1)
	assert.Equal(t, true, lst.Exists(&t1))
	assert.Equal(t, 1, lst.Len())

	t2 := proto.TransferV2{Transfer: proto.Transfer{ID: &d2}}
	lst.Add(&t2)
	assert.Equal(t, true, lst.Exists(&t2))
	assert.Equal(t, true, lst.Exists(&t1))
	assert.Equal(t, 2, lst.Len())

	t3 := proto.TransferV2{Transfer: proto.Transfer{ID: &d3}}
	lst.Add(&t3)
	...
	t4 := proto.TransferV2{Transfer: proto.Transfer{ID: &d4}}
	lst.Add(&t4)
	assert.Equal(t, false, lst.Exists(&t1))
