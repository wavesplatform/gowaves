...
func GetRewardIncreaseDaoXtnPreactivatedTestData(suite *f.BaseSuite) RewardDistributionTestData[RewardDistributionExpectedValuesPositive] {
	return NewRewardDistributionTestData(
		...
		RewardDistributionExpectedValuesPositive{
			MinersSumDiffBalance: int64(utl.GetInitReward(suite)) - 200000000 - 200000000,
...
func GetRewardDecreaseXtnPreactivatedTestData(suite *f.BaseSuite) RewardDistributionTestData[RewardDistributionExpectedValuesPositive] {
	return NewRewardDistributionTestData(
		...
		getAccountPtr(utl.GetAccount(suite, utl.XTNBuyBackAccount)),
		RewardDistributionExpectedValuesPositive{
			MinersSumDiffBalance: int64(utl.GetInitReward(suite)) - int64((utl.GetInitReward(suite)-200000000)/2),
			DaoDiffBalance:       0,
			XtnDiffBalance:       int64((utl.GetInitReward(suite) - 200000000) / 2),
		})
}
