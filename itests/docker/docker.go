...
func (d *Docker) RunContainers(ctx context.Context, paths config.ConfigPaths, suiteName string, goEnv string) (*Ports, error) {
	pwd, err := os.Getwd()
	if err != nil {
		return nil, err
	}
	...
	goNodeRes, goPorts, err := d.runGoNode(ctx, paths.GoConfigPath, suiteName, goEnv)
	if err != nil {
		return nil, err
	}
	d.goNode = goNodeRes
...
func (d *Docker) runGoNode(ctx context.Context, cfgPath string, suiteName string, desireRewardEnv string) (*dockertest.Resource, *PortConfig, error) {
	pwd, err := os.Getwd()
	if err != nil {
		return nil, nil, err
	}
