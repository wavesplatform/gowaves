...
// Positive test for dApp where is checked that account storage is untouched (address)
func (suite *InvokeScriptSuite) Test_CheckThatAccountStorageIsUntouchedAddress() {
	...
	// get test data for invoke script transaction (address string)
	invokeScriptTestData := testdata.GetInvokeScriptAccountStorageUntouchedTestData(&suite.BaseSuite,
		"testdapp", utl.GetAccount(&suite.BaseSuite, newAccount).Address.String())
...
// Positive test for dApp where is checked that account storage is untouched (alias)
func (suite *InvokeScriptSuite) Test_CheckThatAccountStorageIsUntouchedAlias() {
	versions := invoke.GetVersionsInvokeScript(&suite.BaseSuite)
	// create new dApp account with deployed script
	dApp := setscript.CreateDAppAccount(&suite.BaseSuite, utl.DefaultAccountForLoanFunds,
		1000000000, "account_data_storage.ride")
	...
	// set alias for new account
	alias.SetAliasToRecipient(&suite.BaseSuite, "testAcc", utl.GetAccount(&suite.BaseSuite, newAccount))
	// get test data for invoke script transaction (alias string)
