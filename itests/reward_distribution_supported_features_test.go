...
func (suite *RewardDecreaseDaoXtnCeaseXTNBuybackSupportedSuite) Test_NODE826() {
	name := "NODE - 826, NODE-841. XTN buyback reward should be cancelled when CeaseXtnBuyback activated after f19 activation height" +
		" + xtnBuybackRewardPeriod (full reward < 6 WAVES)"
...
func (suite *Reward5W2MinersIncreaseCeaseXTNBuybackSupportedSuite) Test_NODE826_3() {
	name := "NODE - 829, NODE-841. Miner should get full block reward if daoAddress and xtnBuybackAddress are not defined " +
...
func (suite *RewardDaoXtnSupportedWithout20Suite) Test_NODE830() {
	name := "NODE - 830, NODE-841. Block reward distribution should not change after CeaseXtnBuyback activation if CappedReward not activated"
