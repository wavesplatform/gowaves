...
func establishConnections(p *d.Ports) (NodeConnections, error) {
	goCon, err := NewConnection(proto.TCPAddr{}, p.Go.BindHostPort, proto.ProtocolVersion, "wavesL")
	if err != nil {
		return NodeConnections{}, errors.Wrap(err, "failed to create connection to go node")
	}
	scalaCon, err := NewConnection(proto.TCPAddr{}, p.Scala.BindHostPort, proto.ProtocolVersion, "wavesL")
	if err != nil {
