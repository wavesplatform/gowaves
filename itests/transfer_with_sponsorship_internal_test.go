...
func transferWithSponsorshipPositiveChecks(t *testing.T, tx utl.ConsideredTransaction,
...
func transferWithSponsorshipMinAssetFeePositiveChecks(t *testing.T, tx utl.ConsideredTransaction,
...
func transferWithSponsorshipNegativeChecks(t *testing.T, tx utl.ConsideredTransaction,
	...
	for _, v := range versions {
		...
		for name, td := range tdmatrix {
			...
			suite.Run(caseName, func() {
				...
				tx, diffBalances := transfer_utilities.SendTransferTxAndGetBalances(&suite.BaseSuite, td, v, true)
				errMsg := caseName + "Transfer with Sponsorship: " + tx.TxID.String()
				//Checks
				transferWithSponsorshipPositiveChecks(suite.T(), tx, td, diffBalances, errMsg)
			})
		}
	}
}
	...
	for _, v := range versions {
		...
		for name, td := range tdmatrix {
			...
			suite.Run(caseName, func() {
				...
				errMsg := caseName + "Transfer with Sponsorship: " + tx.TxID.String()
				transferWithSponsorshipMinAssetFeePositiveChecks(suite.T(), tx, td, diffBalances, errMsg)
			})
		}
	}
}
	...
	for _, v := range versions {
		...
		for name, td := range tdmatrix {
			...
			suite.Run(caseName, func() {
				...
				errMsg := caseName + "Transfer with Sponsorship: " + tx.TxID.String()
				txIds[name] = &tx.TxID
				transferWithSponsorshipNegativeChecks(suite.T(), tx, td, diffBalances, errMsg)
			})
		}
	}
...
func (suite *TransferWithSponsorshipTxSuite) TestSponsoredTransferFeeNegative() {
	...
	for _, v := range versions {
		//Sponsor creates a new token
		sponsoredAssetId := issue_utilities.IssueAssetAmount(&suite.BaseSuite, testdata.IssueMaxVersion,
...
func (suite *TransferWithSponsorshipTxSuite) TestTransferWithSponsorshipSmokePositive() {
	versions := transfer_utilities.GetVersions(&suite.BaseSuite)
	...
	transfer_utilities.TransferAssetAmount(&suite.BaseSuite, testdata.TransferMaxVersion, utl.TestChainID,
		assetId, testdata.Sponsor, testdata.RecipientSender)
	//Test data
	tdmatrix := utl.GetRandomValueFromMap(testdata.GetSponsoredTransferPositiveData(&suite.BaseSuite,
	...
	for name, td := range tdmatrix {
		...
		suite.Run(caseName, func() {
			//RecipientSender transfers assets to Recipient specifying fee in the sponsored asset
			tx, diffBalances := transfer_utilities.SendTransferTxAndGetBalances(&suite.BaseSuite, td, randV, true)
			errMsg := caseName + "Transfer with Sponsorship: " + tx.TxID.String()
			//Checks
			transferWithSponsorshipPositiveChecks(suite.T(), tx, td, diffBalances, errMsg)
		})
...
func (suite *TransferWithSponsorshipTxSuite) TestTransferWithSponsorshipToOneselfSmokePositive() {
	versions := transfer_utilities.GetVersions(&suite.BaseSuite)
	randV := versions[rand.Intn(len(versions))]
	//Sponsor creates a new token
	sponsoredAssetId := issue_utilities.IssueAssetAmount(&suite.BaseSuite, testdata.IssueMaxVersion, utl.TestChainID,
	...
	assetId := issue_utilities.IssueAssetAmount(&suite.BaseSuite, testdata.IssueMaxVersion, utl.TestChainID,
		testdata.Sponsor, utl.MaxAmount)
	//Test data
	tdmatrix := utl.GetRandomValueFromMap(testdata.GetSposoredTransferBySponsorAsSender(&suite.BaseSuite,
		sponsoredAssetId, assetId))
...
func (suite *TransferWithSponsorshipTxSuite) TestFeeInWavesAccordingMinSponsoredAssetSmokePositive() {
	versions := transfer_utilities.GetVersions(&suite.BaseSuite)
	randV := versions[rand.Intn(len(versions))]
	//Sponsor creates a new token
	sponsoredAssetId := issue_utilities.IssueAssetAmount(&suite.BaseSuite, testdata.IssueMaxVersion, utl.TestChainID,
	...
	for name, td := range tdmatrix {
		...
		suite.Run(caseName, func() {
			//Sponsor set up sponsorship for the token
			sponsor_utilities.SponsorshipEnableSend(&suite.BaseSuite, randV,
				td.TransferTestData.ChainID, sponsoredAssetId, td.MinSponsoredAssetFee)
			//RecipientSender transfers assets to Recipient specifying fee in the sponsored asset
			tx, diffBalances := transfer_utilities.SendTransferTxAndGetBalances(
...
func (suite *TransferWithSponsorshipTxSuite) TestTransferWithSponsorshipSmokeNegative() {
	versions := transfer_utilities.GetVersions(&suite.BaseSuite)
	randV := versions[rand.Intn(len(versions))]
	txIds := make(map[string]*crypto.Digest)
	//Sponsor creates a new token
...
func (suite *TransferWithSponsorshipTxSuite) TestSponsoredTransferFeeSmokeNegative() {
	versions := transfer_utilities.GetVersions(&suite.BaseSuite)
	randV := versions[rand.Intn(len(versions))]
	txIds := make(map[string]*crypto.Digest)
	//Sponsor creates a new token
