...
func TestAddVote(t *testing.T) {
	...
	err := mo.vote(700000000, h, 0, false, blockID0)
	require.NoError(t, err)
	votes, err := mo.newestVotes(h)
	require.NoError(t, err)
	...
	err = mo.vote(500000000, h, 0, false, blockID1)
	require.NoError(t, err)
	votes, err = mo.newestVotes(h)
	require.NoError(t, err)
	assert.Equal(t, uint32(1), votes.increase)
