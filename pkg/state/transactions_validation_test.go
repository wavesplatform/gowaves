...
func validateAndCheck(t *testing.T, to *testObjects, tx proto.Transaction, profileChanges []profileChange) {
	...
	assert.NoError(t, err, "validateBalanceChanges() failed")
	to.stor.flush(t)
...
func TestValidatePayment(t *testing.T) {
	...
	tests := []struct {
		...
		{txTestCase{senderAddr, "", tx.Amount, []block{{timestamp1, blockSig0}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
		// Set insufficient balance for sender with multiple txs in same block.
		{txTestCase{senderAddr, "", tx.Amount * 2, []block{{timestamp1, blockSig0}, {timestamp1, blockSig0}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
...
func TestValidateTransferV1(t *testing.T) {
	...
	tests := []struct {
		...
		// Set insufficient balance for sender and check failure.
		{txTestCase{senderAddr, assetStr, tx.Amount, []block{{timestamp1, blockSig0}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
		// Set insufficient balance for sender with multiple txs in same block.
		{txTestCase{senderAddr, assetStr, tx.Amount, []block{{timestamp1, blockSig0}, {timestamp1, blockSig0}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
		// Set insufficient balance for sender with multiple txs in different blocks.
...
func TestValidateBurnV1(t *testing.T) {
	...
	tests := []struct {
		...
		// Set insufficient balance for sender and check failure.
		{txTestCase{senderAddr, assetStr, tx.Amount - 1, []block{{timestamp1, blockSig0}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
		// Set insufficient balance for sender with multiple txs in same block.
		{txTestCase{senderAddr, assetStr, tx.Amount*2 - 1, []block{{timestamp1, blockSig0}, {timestamp1, blockSig0}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
		// Set insufficient balance for sender with multiple txs in different blocks.
		{txTestCase{senderAddr, assetStr, tx.Amount*2 - 1, []block{{timestamp1, blockSig0}, {timestamp1, blockSig1}}, true, "validateBalanceChanges() did not fail with insufficient balance"}, testNegBalance},
	}
	for _, c := range tests {
		c.f(t, to, tx, &c.txTestCase)
	}
...
func TestValidateLeaseV1(t *testing.T) {
	...
	tests := []struct {
		...
		{leaseTestCase{senderAddr, balanceProfile{tx.Amount, 100500, int64(tx.Amount)}, []block{{timestamp2, blockSig0}}, true, "validateBalanceChanges() did not fail with all balance leased"}, testInvalidLeasing},
		{leaseTestCase{senderAddr, balanceProfile{tx.Amount, 100500, int64(tx.Amount) - 1}, []block{{timestamp2, blockSig0}}, true, "validateBalanceChanges() did not fail with leased balance transfer"}, testInvalidLeasing},
	}
