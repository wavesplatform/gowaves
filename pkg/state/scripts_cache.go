...
type element struct {
	key        string
	value      scripting.Tree
	prev, next *element
	bytes      uint64
}

var defaultValue scripting.Tree
...
func (l *lru) get(key []byte) (value scripting.Tree, has bool) {
	var e *element
...
func (l *lru) set(key []byte, value scripting.Tree, bytes uint64) (existed bool) {
	keyStr := string(key)
	e, has := l.m[keyStr]
	if has {
		l.del(e)
