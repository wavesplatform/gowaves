...
func genTransferWithWavesFee(t *testing.T) *proto.TransferWithProofs {
	...
	tx := proto.NewUnsignedTransferWithProofs(testGlobal.senderInfo.pk, waves, waves, defaultTimestamp, defaultAmount, defaultFee, proto.NewRecipientFromAddress(testGlobal.recipientInfo.addr), &proto.LegacyAttachment{Value: []byte("attachment")})
