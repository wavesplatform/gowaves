...
func createBlockDifferWithSettings(t *testing.T, sets *settings.BlockchainSettings) *blockDifferTestObjects {
	...
	handler, err := newTransactionHandler(sets.Genesis.BlockID(), stor.entities, sets, nil, buildAPIData)
...
func genBlocks(t *testing.T, to *blockDifferTestObjects) (*proto.Block, *proto.Block) {
	...
	parent, err := proto.CreateBlock(
		txs,
		1565694219644,
		randSig,
		testGlobal.matcherInfo.pk,
		proto.NxtConsensus{BaseTarget: 65, GenSignature: genSig},
		proto.NgBlockVersion,
		nil,
		-1,
		proto.TestNetScheme,
		nil,
	...
	child, err := proto.CreateBlock(
		txs,
		1565694219944,
		parent.BlockID(),
		testGlobal.minerInfo.pk,
		proto.NxtConsensus{BaseTarget: 66, GenSignature: genSig},
		proto.NgBlockVersion,
		nil,
		-1,
		proto.TestNetScheme,
...
func TestCreateBlockDiffWithoutNg(t *testing.T) {
	...
	to.stor.addBlock(t, block.BlockID())
	...
	minerDiff, err := to.blockDiffer.createMinerAndRewardDiff(&block.BlockHeader, h, true, block.Transactions)
...
func TestCreateBlockDiffNg(t *testing.T) {
	...
	minerDiff, err := to.blockDiffer.createMinerAndRewardDiff(&child.BlockHeader, hc, true, child.Transactions)
	require.NoError(t, err, "createMinerAndRewardDiff() failed")
	// Verify child block miner's diff.
	correctMinerAssetBalanceDiff := newBalanceDiff(parentFeeNextBlock, 0, 0, 0, false)
...
func TestCreateBlockDiffSponsorship(t *testing.T) {
	...
	err = to.blockDiffer.saveCurFeeDistr(&parent.BlockHeader)
	require.NoError(t, err, "saveCurFeeDistr() failed")
	_, err = to.blockDiffer.createMinerAndRewardDiff(&parent.BlockHeader, hp, false, parent.Transactions)
	require.NoError(t, err, "createMinerAndRewardDiff() failed")
	...
	require.NoError(t, err, "createMinerAndRewardDiff() failed")
	// Verify child block miner's diff.
	correctMinerWavesBalanceDiff := newBalanceDiff(parentFeeNextBlock, 0, 0, 0, false)
...
func TestCreateBlockDiffWithReward(t *testing.T) {
	...
	h2 := to.stor.rw.recentHeight()
	minerDiff, err := to.blockDiffer.createMinerAndRewardDiff(&block2.BlockHeader, h2, true, block2.Transactions)
	require.NoError(t, err)

	fee := defaultFee - defaultFee/5*2
	correctMinerWavesBalanceDiff := newBalanceDiff(int64(fee+to.blockDiffer.settings.InitialBlockReward), 0, 0, 0, false)
...
func TestBlockRewardDistributionWithTwoAddresses(t *testing.T) {
	...
	to.stor.addBlock(t, block2.BlockID())
	h := to.stor.rw.recentHeight()
	minerDiff, err := to.blockDiffer.createMinerAndRewardDiff(&block2.BlockHeader, h, true, block2.Transactions)
	require.NoError(t, err)
	...
	correctFirstRewardAddressBalanceDiff := newBalanceDiff(additionalAddressReward, 0, 0, 0, false)
	correctSecondRewardAddressBalanceDiff := newBalanceDiff(additionalAddressReward, 0, 0, 0, false)
	correctMinerWavesBalanceDiff := newBalanceDiff(fee+reward-2*additionalAddressReward, 0, 0, 0, false)
	correctMinerWavesBalanceDiff.blockID = block2.BlockID()
