...
type balanceDiff struct {
	...
	// Deposit change.
	deposit internal.IntChange[int64]
	blockID proto.BlockID
}

func newBalanceDiff(balance, leaseIn, leaseOut, deposit int64, updateMinIntermediateBalance bool) balanceDiff {
	diff := balanceDiff{
		...
		deposit:                      internal.NewIntChange(deposit),
	}
...
func newMinerFeeForcedBalanceDiff(balance int64, updateMinIntermediateBalance bool) balanceDiff {
	...
	diff := newBalanceDiff(balance, 0, 0, 0, updateMinIntermediateBalance)
...
func (diff *balanceDiff) applyTo(profile balanceProfile) (balanceProfile, error) {
	...
	if newBalance < newLeaseOut+newDeposit && !diff.allowLeasedTransfer {
...
func (td *transactionDiffer) createDiffGenesis(transaction proto.Transaction, _ *differInfo) (txBalanceChanges, error) {
	...
	receiverBalanceDiff, cErr := safecast.Convert[int64](tx.Amount)
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert amount to int64: %w", cErr)
	}
	if err := diff.appendBalanceDiff(
		key.bytes(),
		newBalanceDiff(receiverBalanceDiff, 0, 0, 0, false),
	); err != nil {
...
func (td *transactionDiffer) createDiffPayment(transaction proto.Transaction, info *differInfo) (txBalanceChanges, error) {
	...
	amount, cErr := safecast.Convert[int64](tx.Amount)
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert amount to int64: %w", cErr)
	}
	fee, cErr := safecast.Convert[int64](tx.Fee)
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert fee to int64: %w", cErr)
	}
	senderBalanceDiff := -amount - fee
...
func (td *transactionDiffer) payoutMinerWithSponsorshipHandling(
	...
	issuerAssetBalanceDiff, cErr := safecast.Convert[int64](fee)
	if cErr != nil {
		return fmt.Errorf("failed to convert fee to int64: %w", cErr)
	}
	issuerDiff := newBalanceDiff(issuerAssetBalanceDiff, 0, 0, 0, updateMinIntermediateBalance)
...
func (td *transactionDiffer) convertSponsorAssetToWavesAndDoPayout(
	...
	issuerWavesDiff := newBalanceDiff(issuerWavesBalanceDiff, 0, 0, 0, updateMinIntermediateBalance)
...
func (td *transactionDiffer) createDiffTransfer(tx *proto.Transfer, info *differInfo) (txBalanceChanges, error) {
	...
	if adErr := diff.appendBalanceDiff(
		senderFeeKey,
		newBalanceDiff(senderFeeBalanceDiff, 0, 0, 0, updateMinIntermediateBalance),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	...
	if adErr := diff.appendBalanceDiff(
		senderAmountKey,
		newBalanceDiff(senderAmountBalanceDiff, 0, 0, 0, updateMinIntermediateBalance),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	// Append receiver diff.
...
func (td *transactionDiffer) createDiffEthereumTransferWaves(tx *proto.EthereumTransaction, info *differInfo) (txBalanceChanges, error) {
	...
	fee, cErr := safecast.Convert[int64](tx.GetFee())
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert fee to int64: %w", cErr)
	}
	senderFeeBalanceDiff := -fee
	...
	spErr := td.payoutMinerWithSponsorshipHandling(&changes, false, senderAddress, txFee, txFeeAsset, info)
	if spErr != nil {
		return txBalanceChanges{}, spErr
	}
	return changes, nil
...
func (td *transactionDiffer) createDiffIssue(tx *proto.Issue, id []byte, info *differInfo) (txBalanceChanges, error) {
	...
	if adErr := diff.appendBalanceDiff(
		senderFeeKey.bytes(),
		newBalanceDiff(senderFeeBalanceDiff, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	senderAssetKey := assetBalanceKey{address: senderAddrID, asset: proto.AssetIDFromDigest(assetID)}
	senderAssetBalanceDiff, cErr := safecast.Convert[int64](tx.Quantity)
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert quantity to int64: %w", cErr)
	}
	if adErr := diff.appendBalanceDiff(
...
func (td *transactionDiffer) createDiffExchange(transaction proto.Transaction, info *differInfo) (txBalanceChanges, error) {
	...
	if adErr := diff.appendBalanceDiff(
		senderPriceKey,
		newBalanceDiff(priceAssetDiff, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	...
	if adErr := diff.appendBalanceDiff(
		senderAmountKey,
		newBalanceDiff(-amountDiff, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	...
	senderFee, cErr := safecast.Convert[int64](tx.GetSellMatcherFee())
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert sell matcher fee to int64: %w", cErr)
	}
	senderFeeKey := td.orderFeeKey(senderAddrID, sellOrder)
	if adErr := diff.appendBalanceDiff(
		senderFeeKey,
		newBalanceDiff(-senderFee, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	matcherFeeFromSenderKey := td.orderFeeKey(matcherAddrID, sellOrder)
	if adErr := diff.appendBalanceDiff(
		matcherFeeFromSenderKey,
		newBalanceDiff(senderFee, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	...
	if adErr := diff.appendBalanceDiff(
		receiverFeeKey,
		newBalanceDiff(-receiverFee, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	matcherFeeFromReceiverKey := td.orderFeeKey(matcherAddrID, buyOrder)
	if adErr := diff.appendBalanceDiff(
		matcherFeeFromReceiverKey,
		newBalanceDiff(receiverFee, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	matcherKey := wavesBalanceKey{matcherAddrID}
	matcherFee, cErr := safecast.Convert[int64](tx.GetFee())
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert fee to int64: %w", cErr)
	}
	if adErr := diff.appendBalanceDiff(
		matcherKey.bytes(),
		newBalanceDiff(-matcherFee, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
...
func (td *transactionDiffer) createDiffForExchangeFeeValidation(transaction proto.Transaction, info *differInfo) (txBalanceChanges, error) {
	...
	if adErr := diff.appendBalanceDiff(
		matcherKey.bytes(),
		newBalanceDiff(-matcherFee, 0, 0, 0, true),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	senderFee, cErr := safecast.Convert[int64](tx.GetSellMatcherFee())
	...
	if adErr := diff.appendBalanceDiff(
		senderFeeKey,
		newBalanceDiff(-senderFee, 0, 0, 0, true),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	matcherFeeFromSenderKey := td.orderFeeKey(matcherAddrID, sellOrder)
	...
	if adErr := diff.appendBalanceDiff(
		receiverFeeKey,
		newBalanceDiff(-receiverFee, 0, 0, 0, true),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	matcherFeeFromReceiverKey := td.orderFeeKey(matcherAddrID, buyOrder)
...
func (td *transactionDiffer) createDiffLease(tx *proto.Lease, info *differInfo) (txBalanceChanges, error) {
	...
	senderLeaseOutDiff := amount
	if adErr := diff.appendBalanceDiff(
		senderKey.bytes(),
		newBalanceDiff(0, 0, senderLeaseOutDiff, 0, false)); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	fee, cErr := safecast.Convert[int64](tx.Fee)
	if cErr != nil {
	...
	if adErr := diff.appendBalanceDiff(
		senderKey.bytes(),
		newBalanceDiff(senderFeeDiff, 0, 0, 0, false),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	// Append receiver diff.
...
func (td *transactionDiffer) createDiffLeaseCancel(tx *proto.LeaseCancel, info *differInfo) (txBalanceChanges, error) {
	...
	senderKey := wavesBalanceKey{address: senderAddr.ID()}
	amount, cErr := safecast.Convert[int64](l.Amount)
	if cErr != nil {
		return txBalanceChanges{}, fmt.Errorf("failed to convert amount to int64: %w", cErr)
	}
	senderLeaseOutDiff := -amount
...
func (td *transactionDiffer) createDiffMassTransferWithProofs(transaction proto.Transaction, info *differInfo) (txBalanceChanges, error) {
	...
	for i, entry := range tx.Transfers {
		// Sender.
		amount, caErr := safecast.Convert[int64](entry.Amount)
		if caErr != nil {
			return txBalanceChanges{}, fmt.Errorf("failed to convert amount to int64: %w", caErr)
		}
		senderAmountBalanceDiff := -amount
		if adErr := diff.appendBalanceDiff(
			senderAmountKey,
		...
		recipientBalanceDiff := amount
		if adErr := diff.appendBalanceDiff(
			recipientKey,
			newBalanceDiff(recipientBalanceDiff, 0, 0, 0, updateMinIntermediateBalance),
		); adErr != nil {
			return txBalanceChanges{}, adErr
		}
...
func updateDiffByPayment(
	...
	amount, cErr := safecast.Convert[int64](payment.Amount)
	if cErr != nil {
		return fmt.Errorf("failed to convert amount to int64: %w", cErr)
	}

	var (
		senderPaymentKey  = byteKey(sender, payment.Asset)
		senderBalanceDiff = -amount
		senderDiff        = newBalanceDiff(senderBalanceDiff, 0, 0, 0, updateMinIntermediateBalance)
	)
	...
	var (
		...
		receiverBalanceDiff = amount
		receiverDiff        = newBalanceDiff(receiverBalanceDiff, 0, 0, 0, updateMinIntermediateBalance)
	)
...
func (td *transactionDiffer) createDiffEthereumInvokeScript(tx *proto.EthereumTransaction, info *differInfo) (txBalanceChanges, error) {
	...
	spErr := td.payoutMinerWithSponsorshipHandling(&changes, true, senderAddress, txFee, txFeeAsset, info)
	if spErr != nil {
		return txBalanceChanges{}, spErr
	}
	return changes, nil
...
func (td *transactionDiffer) createFeeDiffInvokeExpressionWithProofs(transaction proto.Transaction, info *differInfo) (txBalanceChanges, error) {
	...
	if adErr := diff.appendBalanceDiff(
		senderFeeKey,
		newBalanceDiff(senderFeeBalanceDiff, 0, 0, 0, true),
	); adErr != nil {
		return txBalanceChanges{}, adErr
	}
	...
	spErr := td.payoutMinerWithSponsorshipHandling(&changes, true, senderAddr, txFee, txFeeAsset, info)
	if spErr != nil {
		return txBalanceChanges{}, spErr
	}
	return changes, nil
...
func (td *transactionDiffer) createFeeDiffEthereumInvokeScriptWithProofs(transaction proto.Transaction, info *differInfo) (txBalanceChanges, error) {
	...
	to := tx.To()
	if to == nil {
		return txBalanceChanges{}, errors.New("empty receiver address in ethereum invoke script transaction")
	}
	scriptAddress, err := to.ToWavesAddress(td.settings.AddressSchemeCharacter)
	if err != nil {
		return txBalanceChanges{}, err
	}
