...
`
	...
	for i := 0; i < b.N; i++ {
		...
		script, _ := messages.BuildScript(r)
		b.StartTimer()
...
func TestDataFunctions(t *testing.T) {
	...
	for _, test := range []struct {
		...
		script, err := messages.BuildScript(reader)
		require.NoError(t, err)
...
func TestFailSript1(t *testing.T) {
	...
	scr, err := messages.BuildScript(r)
	require.NoError(t, err)
...
func TestAssetInfoV3V4(t *testing.T) {
	...
	require.NoError(t, err)
	scriptV3, err := messages.BuildScript(rV3)
	require.NoError(t, err)
	...
	*/
	...
	require.NoError(t, err)
	scriptV4, err := messages.BuildScript(rV4)
	require.NoError(t, err)
