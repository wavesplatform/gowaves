...
func assetInfoToObject(info *proto.AssetInfo) rideType {
	return newRideAsset(
		rideBytes(common.Dup(info.IssuerPublicKey.Bytes())),
		rideBytes(info.ID.Bytes()),
...
func blockInfoToObject(info *proto.BlockInfo) rideType {
	...
	return newRideBlockInfo(
		vrf,
		rideBytes(common.Dup(info.GenerationSignature.Bytes())),
		rideBytes(common.Dup(info.GeneratorPublicKey.Bytes())),
...
func blockHeaderToObject(scheme byte, height proto.Height, header *proto.BlockHeader, vrf []byte) (rideType, error) {
	...
	return newRideBlockInfo(
		vf,
		rideBytes(common.Dup(header.GenSignature.Bytes())),
		rideBytes(common.Dup(header.GenPublicKey.Bytes())),
...
func paymentToObject(scheme byte, tx *proto.Payment) (rideType, error) {
	...
	return newRidePaymentTransaction(
		...
		rideBytes(body),
		rideBytes(tx.ID.Bytes()),
		rideBytes(common.Dup(tx.SenderPK.Bytes())),
		rideInt(tx.Timestamp),
		rideInt(tx.Amount),
		rideInt(tx.Version),
		rideInt(tx.Fee),
...
func orderToObject(scheme proto.Scheme, o proto.Order) (rideType, error) {
	...
	return newRideOrder(
		...
		rideBytes(body),
		rideBytes(id),
		rideBytes(common.Dup(o.GetSenderPKBytes())),
		rideBytes(common.Dup(matcherPk.Bytes())),
		rideInt(o.GetAmount()),
...
func dataEntryToObject(entry proto.DataEntry) rideType {
	...
	case *proto.BinaryDataEntry:
		return newRideBinaryEntry(rideString(entry.GetKey()), rideBytes(e.Value))
...
func invokeExpressionWithProofsToObject(scheme byte, tx *proto.InvokeExpressionTransactionWithProofs) (rideType, error) {
	...
	return newRideInvokeExpressionTransaction(
		...
		rideBytes(body),
		rideBytes(tx.ID.Bytes()),
		rideBytes(common.Dup(tx.Expression.Bytes())),
		rideBytes(common.Dup(tx.SenderPK.Bytes())),
		rideInt(tx.Timestamp),
		rideInt(tx.Version),
		rideInt(tx.Fee),
...
func scriptTransferToObject(tr *proto.FullScriptTransfer) rideType {
	return newRideFullScriptTransfer(
		...
		rideBytes(common.Dup(tr.SenderPK.Bytes())),
		rideBytes(tr.ID.Bytes()),
		rideInt(tr.Amount),
...
func scriptTransferToTransferTransactionObject(st *proto.FullScriptTransfer) rideType {
	return newRideTransferTransaction(
		...
		rideBytes(st.ID.Bytes()),
		rideBytes(common.Dup(st.SenderPK.Bytes())),
		rideInt(st.Amount),
...
func scriptActionToObject(scheme byte, action proto.ScriptAction, pk crypto.PublicKey, id crypto.Digest, timestamp uint64) (rideType, error) {
	...
	case *proto.ReissueScriptAction:
		return newRideReissueTransaction(
			rideUnit{},
			rideUnit{},
			rideBytes(common.Dup(pk.Bytes())),
			rideBytes(a.AssetID.Bytes()),
			rideBytes(id.Bytes()),
			rideInt(timestamp),
			rideInt(0),
			rideInt(a.Quantity),
