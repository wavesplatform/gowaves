...
func TestActionsLimitInOneInvoke(t *testing.T) {
	...
	*/
	...
	firstScript = ""
	...
	*/
	...
	mockState := &MockSmartState{
		...
		NewestAccountBalanceFunc: func(account proto.Recipient, assetID []byte) (uint64, error) {
			...
			case isAssetWaves(assetID):
				return 0, nil
			case account.Address.Equal(dApp1) && bytes.Equal(assetID, asset.Bytes()):
				return 1, nil
			case account.Address.Equal(dApp2) && bytes.Equal(assetID, asset.Bytes()):
				return 0, nil
			}
			return 0, errors.Errorf("unxepected asset '%s'", base58.Encode(assetID))
		},
