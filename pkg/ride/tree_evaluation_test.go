...
func TestFunctionsEvaluation(t *testing.T) {
	...
	envWithDataTX := newTestEnv(t).withTakeStringV5().withTransaction(data)

	exchange := newExchangeTransaction()
	envWithExchangeTX := newTestEnv(t).withTakeStringV5().withTransaction(exchange)
...
func TestInvokeExpression(t *testing.T) {
	...
	*/
	...
	env := newTestEnv(t).withTransaction(byte_helpers.InvokeScriptWithProofs.Transaction.Clone()).
		withLibVersion(tree.LibVersion).toEnv()
	res, err := CallVerifier(env, tree)
	require.NoError(t, err)
...
func TestDataFunctions(t *testing.T) {
	...
	for _, test := range []struct {
		...
		env := newTestEnv(t).withLibVersion(tree.LibVersion).withTransaction(tx).toEnv()
...
func TestMatchOverwrite(t *testing.T) {
	...
	*/
	...
	env := newTestEnv(t).withScheme(proto.TestNetScheme).withHeight(368430).
		withLibVersion(tree.LibVersion).withTransaction(tx).
...
func TestFailSript1(t *testing.T) {
	...
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withTransaction(tx).
...
func TestFailSript2(t *testing.T) {
	...
	*/
	...
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withScheme(proto.TestNetScheme).withHeight(368430).
		withTransaction(tx).
		withDataEntries(
			adminAcc,
...
func TestWhaleDApp(t *testing.T) {
	...
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withHeight(368430).withScheme(proto.MainNetScheme).
...
func TestExchangeDApp(t *testing.T) {
	...
	acc := newTestAccountFromAddress(address)
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withHeight(1642207).withScheme(proto.MainNetScheme).
...
func TestBankDApp(t *testing.T) {
	...
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withHeight(0).withScheme(proto.MainNetScheme).
		withBlock(blockInfo).withWavesBalance(acc, 5000000000).withTransaction(tx).
		withThis(acc).
		withInvokeTransaction(tx).
...
func TestLigaDApp1(t *testing.T) {
	...
	acc := newTestAccountFromAddress(dapp)
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withInvokeTransaction(tx1).withThis(acc).
	...
	te2 := newTestEnv(t).withLibVersion(tree.LibVersion).withInvokeTransaction(tx2).withThis(acc).
		withHeight(1642207).withScheme(proto.TestNetScheme).withBlock(blockInfo).
		withWavesBalance(acc, 3*waves-2*waves-100000-1000000+5-150000, 0, wavesBalance-available).
...
func TestTestingDApp(t *testing.T) {
	...
	acc := newTestAccountFromAddress(address)
	te := newTestEnv(t).withLibVersion(tree.LibVersion).withInvokeTransaction(tx).withThis(acc).
		withHeight(1642207).withScheme(proto.TestNetScheme).withBlock(blockInfo).
...
func TestRecipientAddressToString(t *testing.T) {
	...
	*/
	...
	require.NoError(t, err)
	env := newTestEnv(t).withLibVersion(tree.LibVersion).withMessageLengthV3().withTransactionObject(obj)
...
func TestScriptPaymentPublicKey(t *testing.T) {
	...
	env := newTestEnv(t).withScheme(proto.MainNetScheme).withMessageLengthV3().
		withTransactionObject(scriptTransferToTransferTransactionObject(tr)).withLibVersion(tree.LibVersion)
...
func TestThrowComplexity(t *testing.T) {
	...
	*/
	...
	env := newTestEnv(t).withLibVersion(ast.LibV6).withBlockV5Activated().withProtobufTx().withRideV6Activated().
		withDataEntriesSizeV2().withMessageLengthV3().withValidateInternalPayments().
	...
	_, err = CallFunction(env.toEnv(), tree1, "baz", proto.Arguments{})
	assert.EqualError(t, err, "Explicit script termination")
	assert.Equal(t, 2, EvaluationErrorSpentComplexity(err))

	_, err = CallFunction(env.toEnv(), tree1, "bar", proto.Arguments{})
	assert.EqualError(t, err, "fail")
	assert.Equal(t, 76, EvaluationErrorSpentComplexity(err))

	_, err = CallFunction(env.toEnv(), tree1, "qux", proto.Arguments{})
	assert.EqualError(t, err, "fail")
