...
func initWrappedState(state types.SmartState, env *mockRideEnvironment, rootScriptLibVersion scripting.LibraryVersion) *WrappedState {
	return &WrappedState{
...
func TestAssetInfoV3V4(t *testing.T) {
	...
	treeV3, err := scripting.Parse(srcV3)
	require.NoError(t, err)
	...
	*/
	...
	treeV4, err := scripting.Parse(srcV4)
	require.NoError(t, err)
	assert.NotNil(t, treeV3)
