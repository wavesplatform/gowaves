...
func TestInvokePaymentsCheckBeforeAndAfterInvokeScriptTxActivation(t *testing.T) {
	...
	*/
	...
	t.Run("AfterInvokeScriptActivation", func(t *testing.T) {
		...
		res, err := CallFunction(env.toEnv(), tree1, proto.NewFunctionCall("call", proto.Arguments{}))
		assert.Nil(t, res)
		// also throws user error because of special behavior of scala node
		// also see 'TestEvaluatorComplexityFailedPaymentsCheck/one-invoke-throw/after_light_node' test
		assert.EqualError(t, err, "gotcha")
	})
}
...
func TestEvaluatorComplexityFailedPaymentsCheck(t *testing.T) {
	...
	t.Run("one-invoke-throw", func(t *testing.T) {
		...
		t.Run("after_light_node", func(t *testing.T) {
			env := createEnv(t).withLightNodeActivated()
			// successfully throws custom error because the payments validation error returns after the call
