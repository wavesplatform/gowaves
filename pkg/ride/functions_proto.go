...
func invoke(env Environment, args ...rideType) (rideType, error) {
	...
	// append a call to the custom stack in order to protect a user from the reentrancy attack
	iCall := InvokeCall{funcName: string(fnName), recipient: recipient}
	// push
	ws.invokeCalls = append(ws.invokeCalls, iCall)

	iCallLength := len(ws.invokeCalls)
	iCallLastIndex := len(ws.invokeCalls) - 1
	if iCallLength > 2 {
		if ws.invokeCalls[iCallLastIndex].funcName == ws.invokeCalls[iCallLastIndex-2].funcName &&
			ws.invokeCalls[iCallLastIndex].recipient.Address == ws.invokeCalls[iCallLastIndex-2].recipient.Address {
	...
	res, err := invokeFunctionFromDApp(env, recipient, fnName, listArg)
	// pop
	ws.invokeCalls = ws.invokeCalls[:iCallLastIndex]
