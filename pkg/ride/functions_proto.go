...
func transfer(_ *treeEvaluator, _ environment, args ...rideType) (rideType, error) {
	if err := checkArgs(args, 9); err != nil {
		return nil, errors.Wrap(err, "dataTransaction")
	}
	obj := make(rideObject)
	...
	recipient, err := extractRecipient(args[0])
	if err != nil {
		return nil, errors.Errorf("transfer constructor: unexpected argument type '%s'", args[0].instanceOf())
	}
	obj["recipient"] = rideRecipient(recipient)
	amount, ok := args[1].(rideInt)
	if !ok {
		return nil, errors.Errorf("transfer constructor: unexpected argument type '%s'", args[1].instanceOf())
	}
	obj["amount"] = amount
	return obj, nil
}
