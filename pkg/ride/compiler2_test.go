...
func TestCallExternal(t *testing.T) {
	...
	f, err := compileFunction("", 3, []Node{n}, false)
...
func TestCallWithConstArg(t *testing.T) {
	...
	bt := []byte{
		OpReturn,
		OpRef, 0, 3,
		OpCache, 0, 2, // Function execution code. One line: reference to `v` argument.
		OpPop,
		OpRef, 0, 1,
		OpReturn,
		OpReturn,
...
{-# CONTENT_TYPE EXPRESSION #-}
{-# SCRIPT_TYPE ACCOUNT #-}
func abc(x: String) = true
let y = "7"
abc(y)
*/
func TestCacheFuncArgs(t *testing.T) {
	source := `BAoBAAAAA2FiYwAAAAEAAAABeAYEAAAAAXkCAAAAATcJAQAAAANhYmMAAAABBQAAAAF5eKWK0Q==`
	src, err := base64.StdEncoding.DecodeString(source)
	require.NoError(t, err)
...
/*
@i@func sponsorDeposit(compestart,assetName,compenum) {
	let compeNum = toString(compenum);
	let compeStart = toString(compestart);
	let pmt = extract(i.payment);
	let paymentId = pmt.assetId;
	let currentKey = toBase58String(i.caller.bytes);
	let currentAmount = {
		let $match0 = 1050(this,currentKey);
		if (instanceOf($match0,"Int")) {
			let a = $match0; a
		} else {
			0
		};
	...
	WriteSet(1100(DataEntry((((compeNum + assetName) + compeStart) + currentKey),newAmount),1100(DataEntry((((compeNum + compeStart) + assetName) + currentKey),compestart),nil)))
}
*/
