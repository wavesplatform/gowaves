...
func fractionBigIntRounds(_ Environment, args ...rideType) (rideType, error) {
	...
	pa := p.Abs(p)
	da := d.Abs(&d)
	r, m := pa.DivMod(pa, da, big.NewInt(0))
...
func medianListBigInt(_ Environment, args ...rideType) (rideType, error) {
	...
	half := size / 2
	if size%2 == 1 {
		return rideBigInt(*items[half]), nil
	} else {
		x := items[half-1]
		y := items[half]
