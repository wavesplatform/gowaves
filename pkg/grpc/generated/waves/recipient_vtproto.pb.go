...
func (m *Recipient_PublicKeyHash) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.PublicKeyHash)))
	i--
	dAtA[i] = 0xa
	return len(dAtA) - i, nil
}
...
func (m *Recipient) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			m.Recipient = &Recipient_PublicKeyHash{PublicKeyHash: v}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
			...
			m.Recipient = &Recipient_Alias{Alias: string(dAtA[iNdEx:postIndex])}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
