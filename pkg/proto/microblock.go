...
func (a *MicroBlock) WriteWithoutSignature(scheme Scheme, w io.Writer) (int64, error) {
	...
	fmt.Printf("a.VersionField=%v\n", a.VersionField)
	s.Bytes(a.Reference.Bytes())
	fmt.Printf("a.Reference.Bytes()=%x\n", a.Reference.Bytes())
	s.Bytes(a.TotalResBlockSigField.Bytes())
	fmt.Printf("a.TotalResBlockSigField.Bytes()=%x\n", a.TotalResBlockSigField.Bytes())
	...
	fmt.Printf("uint32(txsBuf.Len() + 4)=%d\n", uint32(txsBuf.Len()+4))
	s.Uint32(a.TransactionCount)
	fmt.Printf("a.TransactionCount=%d\n", a.TransactionCount)
	// Write transactions bytes
	s.Bytes(txsBuf.Bytes())
	fmt.Printf("txsBuf.Bytes()=%x\n", txsBuf.Bytes())
	s.Bytes(a.SenderPK.Bytes())
	fmt.Printf("a.SenderPK.Bytes()=%x\n", a.SenderPK.Bytes())
	println()
	return s.N(), nil
}
