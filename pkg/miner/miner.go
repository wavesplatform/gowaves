...
type MicroblockMiner struct {
	...
	reward int64
	//
	//messageCH chan peer.ProtoMessage
}
...
func (a *MicroblockMiner) mineMicro(ctx context.Context, rest proto.MiningLimits, minedBlock *proto.Block, blocks ng.Blocks, keyPair proto.KeyPair) {
	//select {
	//case <-ctx.Done():
	//	return
	...
	if cnt == 0 {
		go a.mineMicro(ctx, rest, minedBlock, blocks, keyPair)
		return
	}
